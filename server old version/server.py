...
def hex_dump(data, limit=300):  
    if not data:  
        return "(empty)"  
    hex_part = " ".join(f"{b:02x}" for b in data[:limit])  
    ...
    return f"\nHEX:   {hex_part}{truncated}\nASCII: {ascii_part}{truncated}"  
  
  
def hex_dump_full(data):  
    """Full hex dump, 16-byte rows, no truncation."""  
    if not data:  
        return "(empty)"  
    lines_out = []  
    for i in range(0, len(data), 16):  
        ...
        lines_out.append(f"  {i:04x}: {h:<47}  {a}")  
    return "\n" + "\n".join(lines_out)  
...
async def handle_client(reader, writer, db_conn):  
    ...
    try:  
        while True:  
            ...
            logger.info(f"[{addr_str}] Waiting frame header...")  
  
            len_data = await reader.readexactly(4)  
            frame_len = struct.unpack("<I", len_data)[0]  
  
            logger.info(f"[{addr_str}] FRAME LENGTH: {frame_len}")  
  
            encrypted = await reader.readexactly(frame_len)  
            logger.info(f"[{addr_str}] ENCRYPTED RECEIVED: {len(encrypted)} bytes")  
            ...
            logger.info(f"[{addr_str}] DECRYPTED SIZE: {len(raw_msg)}")  
            logger.info(hex_dump(raw_msg))  
            ...
            for k, v in fields.items():  
                logger.info(f"[{addr_str}]   FIELD {k} = {repr(v)}")  
            ...
            for resp_raw in responses:  
                ...
                try:  
                    ...
                    logger.info(hex_dump_full(resp_raw))  
                except Exception:  
        ...
        # Save player state on disconnect (even if no Logout1 was sent)
        ...
        if player and db_conn:
            try:
                ...
                logger.info(f"[{addr_str}] Saved player state on disconnect")
