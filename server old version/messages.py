...
def parse_campaign_attack(data):
    ...
    offset = 0
    try:
        # base: AttackBase (sub-message)
        present, offset = read_field_tag(data, offset)
        if present:
            base = {}
            v, offset = unpack_field_bool(data, offset)
            base["won"] = v
            v, offset = unpack_field_int(data, offset)
            base["seed"] = v
            fields["base"] = base
...
def build_user_info_body(user_id, name, level, diamonds=500, shard_id=1,
...
def build_user_info(user_id, name, level, diamonds=500, shard_id=1):
...
def build_user_extra(player_data, heroes_data):
    ...
    def _ue(label):
        logger.info(f"    [UserExtra] {label} @ offset {len(buf)}")
    ...
    lineups = player_data.get("lineups", {0: [0, 1, 2, 3, 4]})
    buf.extend(build_lineups_columnar(lineups))
    # 19. resources -> Map<ResourceType(enum/int), Integer> V2
    # ResourceType: GOLD=0, DIAMONDS=1, STAMINA=2, XP=3, RAID_TICKETS=10
    _ue("f19 resources (map<enum,int> V2)")
    resources = {
        0:  player_data.get("gold", 50000),
        1:  player_data.get("diamonds", 500),
        2:  player_data.get("stamina", 120),
        3:  player_data.get("xp", 0),
        10: player_data.get("raid_tickets", 5),
    }
...
def build_boot_data(player_data, heroes_data, msg_number, response_to=0,
    ...
    def _snap(label):
        logger.info(f"  [BootData1] {label} @ offset {len(buf)}")
