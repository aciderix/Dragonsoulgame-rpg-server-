    ...
    try:
        ...
        conn.execute(
            ...
            (
                ...
                5, 999,
                "{}",  # campaign_progress starts empty - no pre-seeded levels
                json.dumps(default_lineups), "{}", now, now,
            ),
        )
...
def get_first_player(conn=None):
    """Get the most recent player in the DB (single-player mode).
    Updates last_login timestamp. Returns full player dict with heroes, or None."""
    own_conn = conn is None
    if own_conn:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
    ...
    try:
        row = conn.execute(
            "SELECT * FROM players ORDER BY last_login DESC LIMIT 1"
        ).fetchone()
...
def get_player_heroes(player_id, conn=None):
    ...
    try:
        ...
        for row in rows:
            ...
            # Load equipment JSON (may not exist in old DBs)
            try:
                equip_json = row["equipment"]
            ...
            heroes[hero_num] = {
                ...
                "health": stats.get("health", 1000) if stats else 1000,
            }
        return heroes
    finally:
        if own_conn:
...
def update_player(player_id, conn=None, **kwargs):
    ...
    try:
        ...
        logger.info(f"update_player SQL: {sql} | vals={vals}")
        conn.execute(sql, vals)
        conn.commit()
        logger.info(f"update_player COMMITTED for player_id={player_id}")
    finally:
        if own_conn:
            conn.close()
...
def update_hero(hero_id, conn=None, **kwargs):
    ...
    try:
        ...
        logger.info(f"update_hero SQL: {sql} | vals={vals}")
        conn.execute(sql, vals)
        conn.commit()
        logger.info(f"update_hero COMMITTED for hero_id={hero_id}")
    finally:
        if own_conn:
            conn.close()
...
def save_campaign_progress(player_id, chapter, level, stars, conn=None):
    ...
    try:
        ...
        conn.commit()
        logger.info(f"save_campaign_progress: player={player_id} key={key} "
                     f"stars={stars} total_keys={len(progress)} "
                     f"data={progress_json[:200]}")
        return progress
    finally:
        if own_conn:
            conn.close()
